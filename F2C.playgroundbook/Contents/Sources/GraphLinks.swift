...
public struct Link<T: Particle>: Hashable {
    let a: T
    let b: T
...
public final class Links<T: Particle>: Force {
    ...
    public var links: Set<Link<T>> = []
    public var degrees: Dictionary<T, UInt> = [:]
...
public func ==<T: Particle>(lhs: Link<T>, rhs: Link<T>) -> Bool {
